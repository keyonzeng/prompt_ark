...
export class ShareManager {
    ...
    _redirectToLogin() {
        const loginUrl = `${HUB_URL}/login?source=extension`;
        chrome.tabs.create({ url: loginUrl });
    }

    async _handleNotLoggedIn(action, prompt, platform = null) {
        await this._storePendingIntent(action, prompt);
        ...
        if (socialPlatforms.includes(platform)) {
            ...
            if (platform === 'zhihu' || platform === 'xiaohongshu' || platform === 'wechat') {
                ...
                const fallbackText = platform === 'zhihu'
                    ? this._buildZhihuFallback(title, effectiveUrl, prompt)
                    : platform === 'wechat'
                        ? this._buildWechatFallback(title, effectiveUrl, prompt)
                        : this._buildXiaohongshuFallback(title, effectiveUrl, prompt);
                try {
