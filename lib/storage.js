...
// --- Sync Status Helper ---
export async function updateSyncStatus(state) {
    const status = {
        state,
        lastSyncTime: state === 'synced' ? Date.now() : undefined,
        backend: SyncManager.backend
    };
    try {
...
export const SyncManager = {
    backend: 'none', // 'none' | 'chrome' | 'gist' | 'webdav' | 'obsidian' | 'obsidian-local'
    ...
    async loadConfig() {
        const local = await chrome.storage.local.get([
            'sync_backend', 'gist_id', 'githubToken',
