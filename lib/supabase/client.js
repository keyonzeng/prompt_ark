import { SUPABASE_URL, SUPABASE_ANON_KEY, HUB_URL } from './config.js';
import { 
  initSupabaseClient, 
  setSession, 
  getUser, 
...
export const HubClient = {
  ...
  async fetchPublicPrompts(limit = 50) {
    const { data, error } = await from('prompts')
      .select('*')
  ...
  async publishPrompt(prompt, visibility = 'unlisted') {
    ...
    const { data, error } = await from('prompts').insert(payload).execute();

    if (error) throw error;

    return {
      id: data[0]?.id,
      url: `${HUB_URL}/?prompt=${data[0]?.id}`,
      visibility: visibility,
    };
  },
  ...
  async updateVisibility(promptId, visibility) {
    const url = `${SUPABASE_URL}/rest/v1/prompts?id=eq.${promptId}`;
